...
public class ArbolMViasBusqueda<K extends Comparable<K>, V> implements IArbolBusqueda<K, V> {
    ...
    private boolean hayNodosCompletosNivelN(NodoMVias<K, V> nodoActual, int nivel, int nivelActual) {
        ...
        if (nivelActual == nivel) {
            if ((nodoActual.cantidadDeClavesNoVacias() < this.orden - 1) ||
                    (nodoActual.cantidadDeHijosNovacios() < this.orden)) {
            return false;
            }
            return true;
        }
    ...
    public boolean sonArbolesSimilares(ArbolMViasBusqueda<K,V> arbol2) throws ExeceptionOrdenNoValido {
        if (this.orden != arbol2.orden) {
            throw new ExeceptionOrdenNoValido("Error ambos arboles deben ser del mismo orden");
        }
    ...
    private boolean sonArbolesSimilares(NodoMVias<K, V> nodoActualThisArbol, NodoMVias<K,V> nodoActualArbol2) {
        ...
        if (NodoMVias.esNodoVacio(nodoActualThisArbol) && NodoMVias.esNodoVacio(nodoActualArbol2)) {
            return true;
        }
        
        if (nodoActualThisArbol.cantidadDeClavesNoVacias()!= nodoActualArbol2.cantidadDeClavesNoVacias()) {
            return false;
        }
